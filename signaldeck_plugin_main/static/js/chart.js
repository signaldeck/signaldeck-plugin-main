(() => {
    function normalizeSeries(xVals, yVals, labels, fallbackLabel) {
        const safeXVals = Array.isArray(xVals) ? xVals : [];
        const safeYVals = Array.isArray(yVals) ? yVals : [];
        const xIsMulti = Array.isArray(safeXVals[0]);
        const yIsMulti = Array.isArray(safeYVals[0]);

        const ySeries = yIsMulti ? safeYVals : [safeYVals];
        const xSeries = xIsMulti ? safeXVals : ySeries.map(() => safeXVals);
        const labelSeries = Array.isArray(labels) && labels.length === ySeries.length
            ? labels
            : ySeries.map((_, idx) => ySeries.length === 1 ? fallbackLabel : `${fallbackLabel || "Series"} ${idx + 1}`);

        return { xSeries, ySeries, labelSeries };
    }

    function getChartConfigScatter(ctx, xVals, yVals, unit, labels) {
        let xStepSize = 1000 * 60 * 60 //for intraday, 1 hour
        if (ctx.dataset["agg"] == "day") {
            xStepSize = 1000 * 60 * 60 * 24 * 5 //5 days
        }
        const { xSeries, ySeries, labelSeries } = normalizeSeries(xVals, yVals, labels, ctx.dataset["label"]);
        const datasets = ySeries.map((series, idx) => {
            const data = [];
            const currentX = xSeries[idx];
            for (let i = 0; i < series.length; i++) {
                data.push({ x: currentX[i], y: series[i] });
            }
            return {
                label: labelSeries[idx],
                data: data,
                showLine: true,
                fill: false,
            };
    ...
    function initChart(ctx) {
        ...
        if (ctx.dataset["type"] == "scatter") {
            config = getChartConfigScatter(ctx, xVals, yVals, unit, labels);
        }
